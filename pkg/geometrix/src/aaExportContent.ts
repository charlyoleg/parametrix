...
import { ojscadWriter } from './write_openjscad';
...
// OpenJSCAD
function figureToOjscadF(aCtr: Array<tContour>, faceId: string): string {
	const ojscadW = ojscadWriter();
	for (const ctr of aCtr) {
		ojscadW.addContour(ctr.toOpenjscadSeg());
	}
	const rOjscadF = ojscadW.getFigure(faceId);
	return rOjscadF;
}

type tFaceOjscad = { [index: string]: string };
function makeOpenjscad(geome0: tGeom, designName: string): string {
	const figFaces: tFaceOjscad = {};
	for (const face in geome0.fig) {
		const figu = figureToOjscadF(geome0.fig[face].mainList, `${designName}_${face}`);
		figFaces[face] = figu;
	}
	let rStr = `
...
`;
	...
	rStr += figFaces[faceId2];
	rStr += `
  const subpax_${designName}_${faceId1} = extrudeLinear({height: 600 }, face_${designName}_${faceId1} );
